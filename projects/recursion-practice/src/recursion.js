...
var sumBelow = function(n) {
  ...
  if(n > 0){
    return n -1 + sumBelow(n-1); // when I convert first n -> n-1
  }
  if(n < 0){
    return n +1 + sumBelow(n+1); // when I convert firs n -> n+1
  }
...
var exponent = function(base, exp) {
  ...
  return 1/base*(1/exponent(base,-exp-1));// I made exp + 1 => -exp - 1
  }
...
var powerOfTwo = function(n) {
  ...
  if(n < 1){                //I change n !=== 1 => n < 1 then it passed.
    return false;
  }
...
var palindrome = function(string) {
  string = string.replace(" ","");                                     //pass after replace and toLoweCase methods.
...
var compareStr = function(str1, str2) {
  ...
  if(str1.length === 0 && str2.length ===0) return true; //Base 
  
  if(str1[0] === str2[0]) {
  
  return compareStr(str1.slice(1), str2.slice(1));      //recursion
  }
  return false;
};
...
var rMap = function(array, callback) {
   let [head,...tail] = array;
   if(tail.length === 0) return callback(head);
   
   return [ callback(head) ].concat(rMap(tail, callback));
...
var capitalizeWords = function(input) {
  ...
  var list = capitalizeWords(input.slice(1));
