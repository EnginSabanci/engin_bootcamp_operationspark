...
//     var input = _.indexOf(Array, value);
...
_.unique = function(arr){
    var nonDublicate = [];
    for(var i = 0; i < arr.length; i++){
      if(!_.contains(nonDublicate, arr[i])){
...
_.every = function(coll,func){
     //Return value of calling function would be push results[] array.
    var results = [];
...
_.some = function(coll,func){
   ...
   if (typeof func !== "function"){
      ...
      for(let i = 0; i < coll.length; i++){
        //   for(let j = 0; j < coll.length; j++){
               if(_.contains(falsy, coll[i])){
                   counter++;
               }
        //   }
        } if(counter>0){                                          // I make the counter >0 instead of 1 it passed
            return false;                                         //it was false
            } else {
...
_.reduce = function(arr, func, s){
   ...
   var output;
        if(_.typeOf(s) === "number"){
            ...
            for(let i = 0; i < arr.length; i++){
                if(i === 0){
                    pR = arr[i];                        //The first element is the seed and we assigned it as previous result. We don't pass it into the function
                }else {                                 //like func(arr[i], arr[i], i);  
                    pR = func(pR, arr[i], i); 
                }
            } output = pR;
        } return output;
